...
export default function OfficerPanel({ user, onLogout }) {
  ...
  // keep voterRef in sync so handleTimeoutExpired can access latest voter
  useEffect(() => {
  ...
  // ── poll every 1s when session active ─────────────────────
  useEffect(() => {
    if (sessionActive && voter && !voter.voteProcessed && !timedOut) {
      pollRef.current = setInterval(async () => {
        try {
          ...
          if (res.data.voteProcessed) {
            // ── vote done: stop everything immediately ────────
            stopTimers();
            setCountdown(null);
            setSessionActive(false);
            setTimedOut(false);
            ...
            showMsg(
              `✅ Vote successfully stored for ${res.data.name}!`,
              "success"
            );
          }
        } catch { }
      }, 1000); // 1s polling for fast detection
    }
    return () => clearInterval(pollRef.current);
  }, [sessionActive, voter?.uid, voter?.voteProcessed, timedOut]);
  ...
  useEffect(() => {
    if (sessionActive && voter && !voter.voteProcessed && !timedOut) {
      ...
      // already expired by the time we resume
      if (remaining <= 0) {
        handleTimeoutExpired();
        return;
      }
  ...
  // ── timeout: stop, erase, wait for officer click ──────────
  const handleTimeoutExpired = async () => {
    stopTimers();
    setSessionActive(false);
    setTimedOut(true);
    ...
    const currentVoter = voterRef.current;
    try {
      await axios.post(`${API}/timeout`, { uid: currentVoter?.uid });
    } catch { }
    ...
    showMsg(
      `⏰ Session expired for ${currentVoter?.name}. All partial data cleared. Click Re-Initiate when ready.`,
      "error"
    );
  };
  ...
  const handleSearch = async (e) => {
    ...
    try {
      ...
      } else if (v.voteProcessed) {
        setVoteDone(true);
        showMsg(`Found: ${v.name} — vote already processed`, "success");
      } else {
        showMsg(`Found: ${v.name}`, "success");
      }
    } catch (err) {
          ...
          <div className="voter-result-card">
            ...
            {voteDone && voter.voteProcessed && (
              <div style={{
                ...
                <span style={{ fontSize: 24, marginTop: 2 }}>🎉</span>
                <div>
                  <p className="status-title" style={{ color: "#68d391", marginBottom: 6 }}>
                    Vote Successfully Recorded!
                  </p>
                  <p className="status-desc">
                    H1 received and stored in DB1. RSA encrypted Hash2 saved
                    in DB2. Process complete for{" "}
            ...
            <div className="action-area">
              ...
              {/* Vote done — option to initiate again for same voter */}
              {voter.voteProcessed && (
                <button
                  className="btn initiate"
                  onClick={handleInitiate}
